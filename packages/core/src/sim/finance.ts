...
export function dailyFinance(g: GameState, manager: number, date: { day: number; month0: number; year: number }, rng: Rng, acc?: BalanceSum): FinanceEvent[] {
  const m = g.managers.at(manager);
  ...
  if (komfort > 1 && rng(0, 442 - 52 * komfort) === 0) {
