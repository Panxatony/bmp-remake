...
export function playCupDay(g: GameState, rng: Rng, sim?: MatchSim, zuschauer?: (home: number, away: number) => number | undefined, nachspiel?: NachspielQuelle, vorbereitet = false, nullZwei?: (manager: number) => boolean): CupMatch[] & { finals?: CupFinal[]; gezogen?: number[] } {
  ...
  for (let i = 0; i < n; i++) out.push(playCupMatch(g, 0, 2 * i, false, day, rng, sim, zuschauer, nachspiel, vorbereitet, nullZwei));
